    ...
    for stock_ticker in stocks:  # purchases stocks based on daytrading patterns
        try:
            ...
            if stock_score >= 0.2 and stock_ticker not in all_active_positions.keys():
                ...
                print(
                    "Based on daytrading pattern analysis, buying",
                    stock_ticker,
                    "Stock Score: ",
                    stock_score,
                )
...
def stock_position_analyzer():
    while True:
        for position in active_positions_to_check.keys():
            threading.Thread(
                target=check_perform_sell,
                args=(position, active_positions_to_check[position]),
            ).start()
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # current_time = "12:01"
            if (
                current_time > const.STOCK_MARKET_OPEN_TIME
                and current_time < const.STOCK_MARKET_CLOSE_TIME
            ):
                if first_time_run:
                ...
                for partition in partitioned_stocks:
                    threading.Thread(
                        target=daytrading_stock_analyzer, args=[partition]
                    ).start()
            else:
                ...
                for (
                    stock_ticker
                ) in const.STOCKS_TO_CHECK:  # purchases stocks based on news info
                    threading.Thread(
                        target=news_stock_analyzer, args=(stock_ticker,)
                    ).start()
                time.sleep(360000)
        except Exception as e:
            print("Restarting")
