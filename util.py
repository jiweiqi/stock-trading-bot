...
def calculate_sma(history) -> []:
    ...
    return [
        summation / len(history.index),
        (summation - history.iloc[-2]["Close"]) / (len(history.index) - 1),
    ]
...
def calculate_ema(history) -> int:
    ...
    return history.iloc[-1]["Close"] * weighted_multiplier + sma[1] * (
        1 - weighted_multiplier
    )
